...
export const Header = ({ onMenuToggle }) => {
  ...
  const navigateToOrder = (order) => { navigate(`/orders?viewOrder=${order.id}`); setIsSearchDropdownOpen(false); setSearchQuery(''); };
  const navigateToUser = (user) => { navigate(`/users?viewUser=${user.id}`); setIsSearchDropdownOpen(false); setSearchQuery(''); };
  ...
  const handleNotifClick = (notif) => {
    markAsRead(notif.id);
    navigate(notif.type?.startsWith('TASK_') ? '/tasks' : '/orders');
          ...
          >
            ...
            <div className="max-h-64 overflow-y-auto divide-y divide-border/50">
              {startupUnreadNotifications.map(notif => (
                <button key={notif.id}
                  onClick={() => { markAsRead(notif.id); navigate('/orders'); closeStartupUnreadModal(); }}
        ...
        {isNotifOpen && (
          <div className="absolute right-0 top-full mt-2 w-80 rounded-2xl border border-border bg-card shadow-2xl z-50 overflow-hidden animate-slide-up">
            ...
            <div className="border-t border-border px-4 py-2.5">
              <Link to="/settings" onClick={() => setIsNotifOpen(false)}
