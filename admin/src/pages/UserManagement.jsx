          ...
          >
            {filteredUsers.map(userItem => (
              <motion.div
                ...
                className={`elite-user-card ${userItem.status === 'inactive' ? 'deactivated' : ''}`}
              >
                    ...
                    <div className="avatar-wrapper-elite">
                      <div className="avatar-circle-elite">
                        {userItem.avatar_url ? (
                          ...
                          <div className="initials-elite">
                            {userItem.name?.charAt(0).toUpperCase()}
                          </div>
                        )}
                        <div className={`status-dot ${userItem.status === 'active' ? 'online' : 'offline'}`} />
                      </div>
                    </div>

                    <div className="user-main-info">
                      <h3 className="user-name-elite">{userItem.name}</h3>
                      <span className="user-primary-role">
                        {(userRoles[userItem.id] || [])[0] || 'Member'}
                      </span>
                    </div>
                  </div>

                  <div className="card-actions-popover">
                    <Button variant="ghost" className="dot-menu-btn small">
                      <MoreHorizontal size={20} />
                    </Button>
                    <div className="popover-menu-elite">
                      <button onClick={() => handleEditUser(userItem)}>
                        <Edit2 size={14} /> Edit Profile
                      </button>
                      <button onClick={() => handleConfirmEmail(userItem)} disabled={confirmingUserId === userItem.id}>
                        <Check size={14} /> {confirmingUserId === userItem.id ? 'Confirming...' : 'Confirm Email'}
                      </button>
                      <button onClick={() => handleResetPasswordClick(userItem)}>
                        <Lock size={14} /> Reset Pass
                      </button>
                      <button className="del" onClick={() => handleDeleteUser(userItem)}>
                ...
                <div className="card-details-grid">
                  <div className="detail-item">
                    ...
                    <span className="detail-value">
                      {(userRoles[userItem.id] || [])[0] || 'Staff'} Team
                    </span>
                  </div>
                  <div className="detail-item">
                ...
                <div className="card-contact-info">
                  <div className="contact-row">
                    ...
                    <span className="contact-text">{userItem.email}</span>
                  </div>
                  <div className="contact-row">
